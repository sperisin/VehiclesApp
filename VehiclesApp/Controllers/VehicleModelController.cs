...
namespace VehiclesApp.MVC.Controllers
{
    public class VehicleModelController : Controller
    {
        ...
        public VehicleModelController(IMapper mapper, AppDbContext appDbContext, IVehicleModelRepository _vehicleModelRepository, IVehicleMakeRepository _vehicleMakeRepository)
        {
            this._mapper = mapper;
        ...
        [HttpPost]
        public async Task<RedirectToActionResult> Create(VehicleModelVM vehicleModelVM)
        {
            VehicleMake vehicleMake = await _vehicleMakeRepository.GetVehicleMakeAsync(vehicleModelVM.vehicleMakeId);
            VehicleModel vehicleModel = new VehicleModel()
            { 
                Id = vehicleModelVM.Id,
                Name = vehicleModelVM.Name,
                Abrv = vehicleModelVM.Abrv,
                vehicleMake = vehicleMake,
                vehicleMakeId = vehicleModelVM.vehicleMakeId
             };
        ...
        public async Task<IActionResult> Index([FromQuery] int? page, int? pagesize, string search, string sort, string direction, int make)
        {
            var sortParameters = new SortParameters()
            {
                Sort = sort,
            ...
            VehicleMake vehicleMake = await _vehicleMakeRepository.GetVehicleMakeAsync(make);
            var vehicleModelList = await _vehicleModelRepository.GetVehicleModelsAsync(sortParameters, filterParameters, pagingParameters, vehicleMake);
